...
Together, these design choices yield \textbf{10--1{,}000$\times$} speedup in data-to-state latency depending on encoding method, qubit count, and baseline framework, as we demonstrate in our interactive system.
...
QDP's advantage is consistent across all methods, ranging from 9--24$\times$ over \texttt{lightning.gpu} to over 1{,}000$\times$ over \texttt{default.qubit}, depending on encoding and qubit count.
...
By implementing the full path from columnar storage to GPU-resident quantum states in Rust and CUDA, Apache Mahout QDP achieves 24$\times$ speedup over PennyLane's GPU-accelerated \texttt{lightning.gpu} backend at 20 qubits, and 10--1{,}000$\times$ over CPU-based backends depending on encoding method.
