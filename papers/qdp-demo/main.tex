...
QDP consistently achieves the lowest latency, even against PennyLane's GPU-accelerated \texttt{lightning.gpu}.
For angle encoding at 20 qubits, QDP (0.27\,ms) is 1{,}018$\times$ faster than PennyLane (274\,ms) and 24$\times$ faster than \texttt{lightning.gpu} (6.5\,ms).
Qiskit's \texttt{initialize()} grows exponentially, reaching 171\,ms at 10 qubits while QDP remains at 0.11\,ms.
...
\textbf{Correctness.}
Since QDP uses fused CUDA kernels rather than gate simulation, we validate correctness by comparing encoded states against Qiskit across all three encoding methods and 2--20 qubits.
\autoref{tab:correct} shows the maximum amplitude difference is below $10^{-16}$ (machine epsilon) in every case.
...
We thank the Mahout community and the Apache Taipei\footnote{\url{https://github.com/opensource4you}} community for their support.
\end{acks}
